...
## Complex example
To illustrate the flexibility of `make_boot_pmtable()`, we'll use a more complex example. Here we'll include two confidence intervals, the median value, and an extra 25% quantile (just for fun). Individual percentiles must be named `boot_perc_[x]` for `make_boot_pmtable` to format the names correctly, though custom adjustments can be done using `pmtables::st_rename()`.
...
param_df <- define_param_table(
  ...
  # Retain parameter_names column for joining
...
boot_df2 <- define_boot_table(
  .boot_estimates = bbr::bootstrap_estimates(boot_run),
  .key = paramKeyPath,
  .ci = "iqr"
) %>% format_boot_table()

# Add custom percentile

perc_25 <- bbr::summarize_bootstrap_run(boot_run) %>%
  bbr::param_estimates_compare(probs = 0.25) %>%
  dplyr::transmute(
    parameter_names, 
    boot_perc_25 = signif(p25, 3)
  )


boot_param_df <- param_df %>%
  left_join(perc_25, by = "parameter_names") %>%
  left_join(boot_df, by = c("abb", "desc")) %>%
  left_join(boot_df2, by = c("abb", "desc", "boot_value"))
```
...
tab <- make_boot_pmtable(.df = boot_param_df, .pmtype = "fixed") %>% 
  ...
  stable(output_file = "deliv/mod106-boot-95-iqr-ci-25-perc-fixed.tex")
