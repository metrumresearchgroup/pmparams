...
tableList$bootstrap %>% stable() %>% st2report()
...
bootParam = left_join(nonboot_df2, df2, by = c("abb", "desc"))

head(bootParam)


footBoot = glue::glue("CI were determined from the 2.5th and 97.5th percentiles 
                of the ({nrow(df2)}) non-parametric bootstrap estimates.")

#Fixed
tableList$fixed <- make_boot_pmtable(.boot_df = df2,
                    .nonboot_df = nonboot_df2,
                    .pmtype = "fixed") %>% 
