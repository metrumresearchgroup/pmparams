...
# Creating bootstrap estimate parameter table

### Bootstrap parameter estimates argument
`pmparams` allows for different bootstrap parameter estimate input types:

* A ***path*** to a csv (`file.path(model_dir, "boot/data/boot-106.csv")`)
* A data.frame (like what's returned from `bbr::bootstrap_estimates()`)

As of `bbr 1.11.0`, bootstrapping can be done in-house, tracking a new bootstrap model object (`boot_run`). Once executed, we can easily read in these estimates using `bbr::bootstrap_estimates()`.
...
footBoot <- glue::glue("CI were determined from the 2.5th and 97.5th percentiles 
                of the non-parametric bootstrap estimates (n ={nrow(boot_ests)}).")
...
boot_df2 <- boot_ests %>%
  define_boot_table(param_key, .ci = "iqr") %>% 
  format_boot_table()
