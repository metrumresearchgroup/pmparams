...
footBoot = glue::glue("CI were determined from the 2.5th and 97.5th percentiles 
                of the non-parametric bootstrap estimates (n ={nrow(read.csv(boot_paramEstPath))}).")

#Fixed
# tableList$fixed1 <- make_boot_pmtable(.df = df2,
#                     .pmtype = "fixed") %>% 
...
tableList$fixed2 <- make_boot_pmtable(.df = bootParam,
                    .pmtype = "fixed") %>% 
  st_center(desc = col_ragged(5.5),
            abb = "l") %>% 
  st_notes(param_notes()$ci) %>%         # add abbreviations
