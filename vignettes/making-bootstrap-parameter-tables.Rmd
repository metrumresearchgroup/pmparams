...
# Render table
boot_df %>% mutate(
  ci_95 = paste(`perc_97.5`, ",", `perc_2.5`)
) %>%
  select(-c("perc_2.5", "perc_97.5")) %>%
  st_new() %>% 
  st_panel("desc") %>% 
  st_span("Non-parametric bootstrap", c("perc_50", "ci_95")) %>%
  st_rename(
    Median = perc_50,
    "95\\%CI" = ci_95,
    " "= abb
...
make_boot_pmtable(.df = boot_param_df, .pmtype = "random", .show_desc = FALSE) %>% 
  st_center(desc = col_ragged(5.5), abb = "l") %>% 
  st_notes(param_notes()$ci) %>%                   # add abbreviations
  st_notes_str() %>%                               # collapse all abbreviations to a string                        
  st_notes(param_notes()$logTrans) %>%   # customize other notes
