...
```{r, fig.align='center'}
st_image_show(tab_out, width = 0.4)
```
...
```{r, eval = eval_bbr_chunks, fig.align='center'}
# Define footnote
...
# Render table
make_boot_pmtable(.df = boot_param_df, .pmtype = "fixed") %>%
  ...
  stable() %>% 
  st_as_image(width = 0.4, border = "0.2cm 0.7cm")
```
...
make_boot_pmtable(.df = boot_param_df, .pmtype = "fixed") %>% 
  ...
  stable() %>% 
  st_as_image(width = 0.4, border = "0.2cm 0.7cm 5.2cm 0.7cm")
```

```{r, eval = eval_bbr_chunks, fig.align='center'}
##  RANDOM EFFECTS table ----------------------------
make_boot_pmtable(.df = boot_param_df, .pmtype = "random") %>% 
  st_center(desc = col_ragged(5.5), abb = "l") %>% 
  st_notes(param_notes()$ci) %>%                   # add abbreviations
  st_notes_str() %>%                               # collapse all abbreviations to a string                        
  st_notes(param_notes()$logTrans) %>%   # customize other notes
  ...
  stable() %>% 
  st_as_image(width = 0.4, border = "0.2cm 0.7cm 3.2cm 0.7cm")
```
