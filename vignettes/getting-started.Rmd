...
#fixed footnotes
footNote1 <- "Abbreviations: CI = confidence intervals; SE = standard error"
footNote2 <- "Confidence intervals = estimate $\\pm$ 1.96 $\\cdot$ SE"

#random footnotes
footNote3 <- "Abbreviations: CI = confidence intervals; Corr = Correlation coefficient; CV = coefficient of variation; SD = standard deviation; SE = standard error"
footNote4 <- "CV\\% of log-normal omegas = sqrt(exp(estimate) - 1) $\\cdot$ 100" 
footNote5 <- "CV\\% of sigma = sqrt(estimate) $\\cdot$ 100"
```

```{r}
##fixed
tableList$fixed <- make_pmtable(df2, .pmtype = "fixed", .notes = c(footNote1, footNote2))
...
```{r}
##random
tableList$random <- make_pmtable(df2, .pmtype = "random", .notes = c(footNote3, footNote4, footNote5))
