...
![Exported functions](../man/figures/test.png){width=75%}
...
- runs `checkTransforms()`, `defineRows()`,& `getValueSE`
...
- If a user wants to use a CI that is NOT 90% or 95%, they will need to provide that CI's z-score in the `.zed` argument
...
- user can specify desired columns but default columns are: `c("type", "abb", "greek", "desc", "value", "ci", "shrinkage")`
- user can output all columns by specifying `.select_col` = "all"
- user can specify if they want to output the `pRSE` column. Default does not output `pRSE` column

# INTERNAL FUNCTIONS- Keep notes for now; remove later
