...
test_that("checkTransforms tildes input: works with trans column with no tildes [MPT-CTF-002]", {
  expect_equal(newDF %>% dplyr::mutate(transTHETA = NA), checkTransforms(newDF))
})

test_that("checkTransforms tildes input: works with trans column with tilde [MPT-CTF-002]", {
  newDF2 <- newDF
