...
test_that("make_boot_pmtable pmtable commands: st_panel", {
  pm_tibble1 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "full")
...
test_that("make_boot_pmtable pmtable commands: make cols blank", {
  ...
  pm_tibble2 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "random")
  expect_equal(unname(unlist(pm_tibble2$cols_blank)), c("abb", "greek"))
})

test_that("make_boot_pmtable pmtable commands: rename cols", {
  pm_tibble3 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "structural")
  expect_equal(unname(unlist(pm_tibble3$cols_rename[1])), "value")

  pm_tibble4 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "random")
...
test_that("make_boot_pmtable pmtable commands: notes", {
  pm_tibble5 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "fixed") %>% pmtables::st_notes(c("note 1", "note2"))
  pm_tibble6 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "fixed", .width = 2.1) %>% pmtables::st_notes(c("note 1", "note2"))
...
test_that("make_boot_pmtable pmtable commands: args", {
  pm_tibble5 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "fixed")
...
test_that("make_boot_pmtable correctly filters with .pmtype", {
  pm_tibble4 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "random")
  pm_tibble5 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "fixed")
  pm_tibble6 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "full")
  pm_tibble7 <- make_boot_pmtable(.df = BOOT_PARAM_TAB_106, .pmtype = "structural")
...
test_that("make_boot_pmtable: intervals and percentiles are appropriately grouped", {
  ...
  boot_complete1 <-  left_join(FMT_PARAM_TAB_106, boot_df2, by = c("abb", "desc"))
